...
manual_data = np.loadtxt(
    "calibration/2024-03-18_0829 radiative heating test.csv", delimiter=",", skiprows=9
)
pyro_data = np.loadtxt("calibration/2024-03-18_08-28-18.tsv", skiprows=1)

photocurrent_interp = interp1d(pyro_data[:, 0], pyro_data[:, 1])

photocurrents = photocurrent_interp(manual_data[:, 0])
filament_photocurrent = linear(manual_data[:, 2], *popt_cold_linear)

plt.plot(manual_data[:, 0], photocurrents, label="Total Photocurrent")
plt.plot(manual_data[:, 0], filament_photocurrent, label="Filament Contribution")
plt.xlabel("Time (s)")
plt.ylabel("Photodiode Current (A)")
plt.title("Significance of Filament Background")
plt.legend()
plt.show()
